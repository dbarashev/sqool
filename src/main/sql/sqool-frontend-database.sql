...
CREATE OR REPLACE VIEW ContestDto AS
  SELECT code, name, lower(dates) AS start_ts, upper(dates) AS end_ts
  FROM Contest;
...
-- Variant is a part of contest and has a name which is unique in the contest scope
...
CREATE TABLE TaskVariant(
  id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  task_id INTEGER REFERENCES Contest.Task ON DELETE CASCADE ON UPDATE CASCADE,
  variant_id INTEGER REFERENCES Contest.Variant ON DELETE CASCADE ON UPDATE CASCADE,
  UNIQUE(task_id, variant_id)
);

CREATE OR REPLACE VIEW VariantDto AS
SELECT id, name,
       array_to_json(ARRAY(
         SELECT task_id FROM Contest.TaskVariant WHERE variant_id = Contest.Variant.id

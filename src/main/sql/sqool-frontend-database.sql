...
CREATE TABLE Variant(
  ...
  contest_code TEXT NOT NULL REFERENCES Contest ON DELETE CASCADE ON UPDATE CASCADE,
  num INT NOT NULL,
  UNIQUE(contest_code, num)
);

CREATE OR REPLACE VIEW VariantDto AS
SELECT id, contest_code AS contest, num
FROM Variant;

CREATE OR REPLACE FUNCTION VariantDto_Insert()
  RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO Variant(contest_code, num)
  VALUES (NEW.contest, NEW.num);
  RETURN NEW;
end;
$$ LANGUAGE plpgsql;
